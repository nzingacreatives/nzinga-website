...
function render(){
 ...
 const data=read().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)).filter(s=>{const t=s.title||s.messages?.find(m=>m.role==='user')?.content||'Nova conversa';return !term||t.toLowerCase().includes(term)});
 list.innerHTML=data.length?data.map(s=>{const t=s.title||s.messages?.find(m=>m.role==='user')?.content||'Nova conversa';const safe=String(t).slice(0,60).replace(/[&<>]/g,'');return '<div class="gpt-history-item"><button class="gpt-history-open" type="button" data-open="'+s.id+'"><span class="gpt-history-title">'+safe+'</span><small>'+dateLabel(s.updatedAt||s.createdAt)+'</small></button><button class="gpt-history-rename" type="button" data-rename="'+s.id+'" aria-label="Renomear conversa">✎</button><button class="gpt-history-delete" type="button" data-delete="'+s.id+'" aria-label="Apagar conversa">×</button></div>'}).join(''):'<div class="gpt-history-empty">'+(term?'Nenhuma conversa encontrada.':'Ainda não há conversas guardadas.')+'</div>';
 ...
 list.querySelectorAll('[data-rename]').forEach(b=>b.onclick=()=>{const data=read();const s=data.find(x=>x.id===b.dataset.rename);if(!s)return;const current=s.title||s.messages?.find(m=>m.role==='user')?.content||'Nova conversa';const next=prompt('Nome da conversa:',current);if(next&&next.trim()){s.title=next.trim().slice(0,80);write(data);render()}});
...
function initComposer(){
 ...
 if(Recognition){
  const mic=document.createElement('button');mic.type='button';mic.className='gpt-voice-btn';mic.title='Falar para escrever';mic.setAttribute('aria-label','Falar para escrever');mic.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><rect x="8" y="3" width="8" height="12" rx="4" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5 11a7 7 0 0 0 14 0M12 18v3M9 21h6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
...
function addMessageTools(){
 ...
 const pickVoice=()=>{if(!('speechSynthesis'in window))return null;const voices=speechSynthesis.getVoices();const saved=localStorage.getItem('nzingaGPT.voice');if(saved){const v=voices.find(x=>x.name===saved);if(v)return v}const pt=voices.filter(v=>/^pt(-|_|$)/i.test(v.lang));const maleHints=/ant[óo]nio|antonio|duarte|jorge|jo[aã]o|ricardo|daniel|miguel|rui|carlos|male|masculino/i;const male=pt.find(v=>maleHints.test(v.name));const chosen=male||pt[0]||voices[0]||null;if(chosen)try{localStorage.setItem('nzingaGPT.voice',chosen.name)}catch{}return chosen};const speak=t=>{if(!('speechSynthesis'in window))return;speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(t);u.lang='pt-PT';u.rate=.98;const v=pickVoice();if(v)u.voice=v;speechSynthesis.speak(u)};if('speechSynthesis'in window)speechSynthesis.onvoiceschanged=()=>pickVoice();
 ...
 const decorate=b=>{if(!b.classList.contains('assistant')||b.dataset.toolsReady||!b.textContent.trim()||b.textContent.trim()==='...')return;b.dataset.toolsReady='1';
  const bar=document.createElement('div');bar.className='gpt-message-tools';bar.innerHTML='<button type="button" data-a="like" title="Gostar" aria-label="Gostar"><svg viewBox="0 0 24 24"><path d="M7 10v10H4V10h3Zm2 10h7.2a2 2 0 0 0 1.9-1.4l2.1-6.4A1.7 1.7 0 0 0 18.6 10H15l.5-3.2A2.4 2.4 0 0 0 13.1 4L9 10v10Z" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/></svg></button><button type="button" data-a="copy" title="Copiar" aria-label="Copiar"><svg viewBox="0 0 24 24"><rect x="8" y="8" width="11" height="11" rx="2" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M16 8V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2" fill="none" stroke="currentColor" stroke-width="1.8"/></svg></button><button type="button" data-a="listen" title="Ouvir" aria-label="Ouvir"><svg viewBox="0 0 24 24"><path d="M5 10v4h4l5 4V6l-5 4H5Zm12-1.5a4 4 0 0 1 0 7M18.5 6a7 7 0 0 1 0 12" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button><button type="button" data-a="share" title="Partilhar" aria-label="Partilhar"><svg viewBox="0 0 24 24"><path d="M12 16V4m0 0L7 9m5-5 5 5M5 14v5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button><button type="button" data-a="regen" title="Regenerar resposta" aria-label="Regenerar resposta"><svg viewBox="0 0 24 24"><path d="M20 11a8 8 0 1 0 1 4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M20 5v6h-6" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button>';b.appendChild(bar);
...
function init(){
 ...
 const startToggle=document.getElementById('gptStartToggle'),startOptions=document.getElementById('gptStartOptions');startToggle?.addEventListener('click',()=>{const open=!startOptions.hidden;startOptions.hidden=open;startToggle.textContent=open?'＋':'−';startToggle.setAttribute('aria-expanded',String(!open))});document.querySelectorAll('.starter-inline').forEach(b=>b.addEventListener('click',()=>{const i=document.getElementById('gptPrompt');i.value=b.textContent;i.focus();i.dispatchEvent(new Event('input'))}));document.querySelectorAll('[data-gpt-mode]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-gpt-mode]').forEach(x=>x.classList.remove('active'));b.classList.add('active');const map={ideia:'Quero explorar uma ideia comigo. Ajuda-me a desenvolver possibilidades concretas.',aprender:'Quero aprender sobre um assunto. Explica de forma clara e por etapas.',organizar:'Quero organizar uma tarefa ou problema. Ajuda-me a definir prioridades e passos.',escrever:'Quero escrever ou melhorar um texto. Ajuda-me a criar uma versão pronta para editar.'};const i=document.getElementById('gptPrompt');i.value=map[b.dataset.gptMode]||'';i.focus();i.dispatchEvent(new Event('input'))}));
}
