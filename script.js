(() => {
  ...
  function initMinhaNzingaAuth(){
    ...
    if(!form || form.dataset.nzingaAuthReady==='1') return;
    form.dataset.nzingaAuthReady='1';
    ...
    if(!signup){
      ...
      signup.id='signupBtn'; signup.type='button'; signup.className='button'; signup.textContent='Criar conta';
      form.appendChild(signup);
    }
    const message=(text,error=false)=>{if(status){status.textContent=text;status.style.color=error?'#ffb3b3':'#fff';}};
    ...
    signup.addEventListener('click',async()=>{
      try{
        ...
        signup.disabled=true; signup.textContent='A criar conta…'; message('A criar a tua conta…');
