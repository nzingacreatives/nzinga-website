...
(function(){
  function boot(){
    ...
    /* Capture the logout click before any older inline handler can interfere. */
      ...
      sb.auth.signOut({scope:'local'}).then(function(result){
        ...
        try{
          Object.keys(localStorage).forEach(function(key){
            if(key.indexOf('sb-')===0||key.indexOf('supabase')!==-1)localStorage.removeItem(key);
          });
        }catch(e){}
        window.location.replace('minha-nzinga.html?logged_out=1');
      }).catch(function(error){
        console.error('Nzinga logout error:',error);
        target.disabled=false;
        target.textContent='Sair da conta';
    ...
    sb.auth.getSession().then(function(result){
      ...
      /* Show the admin entry only to users present in admin_users. */
      sb.from('admin_users').select('role').eq('user_id',session.user.id).maybeSingle().then(function(adminResult){
        ...
        link.id='adminPanelBtn';
        link.href='admin.html';
        link.className='button';
        link.textContent='Painel Admin →';
        link.style.marginTop='10px';
        if(logout&&logout.parentElement)logout.parentElement.insertBefore(link,logout);
